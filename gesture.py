...
import numpy as np
from pycaw.pycaw import AudioUtilities, IAudioEndpointVolume #audio
from comtypes import CLSCTX_ALL
from math import hypot # measure euclidean dist
...
# real-time tracking of hands using mediapipe(21 land marks)
mp_hands = mp.solutions.hands
hands = mp_hands.Hands(min_detection_confidence=0.7, min_tracking_confidence=0.7) # if the model is 70% confident
mp_draw = mp.solutions.drawing_utils

# Initialize Pycaw for volume control
devices = AudioUtilities.GetSpeakers()
device = devices  # Correct way to get the first available device
interface = device.Activate(IAudioEndpointVolume._iid_, CLSCTX_ALL, None) #CLSCTX is for local server, remote server etc
...
def video_processing():
    ...
    while camera_started:
        ...
        if results.multi_hand_landmarks: #differentiate between left and right hands
